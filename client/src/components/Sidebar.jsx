...
const Sidebar = () => {
  ...
  const navItems = [
    ...
    role === "ADMIN"
      ...
      : { name: "Attendance", href: "/attendance", icon: UserIcon },
  ...
  const sidebarContent = (
    <>
      ...
      {/* User Profile Card */}
      {userName && (
        <div className="mx-3 mb-1 p-3 rounded-lg bg-white/3 border border-white/4">
          <div className="flex items-center gap-3">
            <div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.name}
              to={item.href}
              className={`group flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-medium transition-all duration-150 relative ${isActive ? "bg-ingigo-500/12 text-indigo-300" : "text-slate-300 hover:text-white hover:bg-white/4"}`}
            >
              {isActive && (
  ...
  return (
    <div>
      ...
      <aside
        className={
          'lg:hidden fixed inset-y-0 left-0 w-72 bg-linear-to-b from-slate-900 via-slate-900 to-slate-950 text-white z-50 flex-col transform transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full" } '
        }
      >
        {sidebarContent}
      </aside>
